...
def get_model(args, device='cuda'):
    ...
    if args.model == "mnist_cnn":
        ...
        if args.dataset == "eurosat":
            global_model = EuroSATResNet(num_classes=10).to(device)
        elif args.dataset == "siri-whu":
            global_model = SIRINet(num_classes=12).to(device)
        else:
            num_classes = 200
...
def main():
    ...
    if args.type == "pretrain":
        ...
        # Search for available checkpoints to find the latest valid step
        import re
        initial_step = args.local_ep # fallback
        try:
            # Pattern to match: {dataset}_{num_users}clients_{beta}_{step}epoch_{seed}_client0.pkl
            # We look for client 0 specifically to enumerate steps. Escape beta just in case.
            # Using simple string matching parts to be safer against regex special chars in args
            prefix = f"{args.dataset}_{args.num_users}clients_{args.beta}_"
            suffix = f"epoch_{args.seed}_client0.pkl"
            
            steps_found = []
            if os.path.exists(args.weights_dir):
                for fname in os.listdir(args.weights_dir):
                    if fname.startswith(prefix) and fname.endswith(suffix):
                        # Extract the middle part: {step}
                        # fname = prefix + step + suffix
                        step_str = fname[len(prefix):-len(suffix)]
                        if step_str.isdigit():
                            steps_found.append(int(step_str))
            
            if steps_found:
                initial_step = max(steps_found)
                print(f"Auto-detected latest checkpoint step: {initial_step}")
            else:
                 print(f"No exact matching checkpoint pattern found in {args.weights_dir}. Using provided local_ep: {initial_step}")

        except Exception as e:
             print(f"Warning during checkpoint search: {e}")

        # Try to load initial weights
        print(f"Attempting to load weights for step: {initial_step}")
        loaded_count = 0
        for k in range(args.num_users):
