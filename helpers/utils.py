...
def kldiv(logits, targets, T=1.0, reduction='batchmean'):
    ...
    # if torch.any(p < 0):
    #     print("Warning: Negative values detected in p")
    # if torch.any(p < 1e-35):
    #     print(f"Warning: Extremely small values detected in p (min: {p.min().item()})")
    # if torch.any(torch.isnan(q)):
    #     print("Warning: NaN values detected in q")
    # print(f"q: min {q.min().item()}, max {q.max().item()}")
    # print(f"p: min {p.min().item()}, max {p.max().item()}")
    p = torch.clamp(p, min=1e-40)  # 避免 p 中有接近0的值
    # print(F.kl_div(q, p, reduction=reduction) * (T * T))
    return F.kl_div(q, p, reduction=reduction) * (T * T)

def js_div(logits, targets, T=1.0, reduction='batchmean'):
...
def save_image_cls(imgs,target,dir,cls_num,batch_idx):
    if isinstance(imgs, torch.Tensor):
        imgs = (imgs.detach().clamp(0, 1).cpu().numpy() * 255).astype('uint8')
    ...
    for idx, img in enumerate(imgs):
        if img.shape[0] == 1:
            ...
            img = Image.fromarray(img.transpose(1, 2, 0))
        img.save(os.path.join(base_dir,str(int(target[idx]))+'/%d-%d.png'%(batch_idx,idx)))
...
class ImagePool(object):
    ...
    def add(self, imgs, targets=None):
        ...
        save_image_cls(imgs,targets,os.path.join(self.root, "%d.png" % (self._idx)),self.cls_num,self._idx)
        self._idx += 1
